...
(require racket/match racket/string json)
...
(define (run-module m ctx inputs send! #:trace [tr #f] #:cache? [cache? #t])
  ...
  (define prompt 
    (if (null? images)
        (render-prompt m ctx inputs)
        (list (hash 'type "text" 'text (render-prompt m ctx inputs))
  ...
  (define outs (if (hash? parsed)
                   (for/hash ([f (Signature-outs (Module-sig m))])
