...
(require racket/match
         ...
         ;; Service Client for Auth
         (only-in "../service/client.rkt"
                  connect-service!
                  client-login!
                  client-register!
                  client-create-session!)
         ;; Core functionality for feature parity
         (only-in "../core/commands.rkt"
...
(struct model (state
               ...
               api-key     ; string
               ;; Auth State
               auth-mode   ; login/register/choice
               auth-field  ; current focused field
               email-input ; text-input-model
               password-input ; text-input-model
               display-name-input ; text-input-model
  ...
  (lambda ()
    ...
    (define initial-model
      (model STATE-INTRO
             ...
             AUTH-CHOICE
             'email
             (text-input-init #:placeholder "user@example.com" #:prompt "Email: ")
             (text-input-init #:placeholder "********" #:prompt "Password: " #:mask-char #\*)
             (text-input-init #:placeholder "Your Name" #:prompt "Display Name: ")
             #f ; error-message
             #f)) ; service-client
    ...
    ;; Start connecting service if URL provided?
    ;; For now main-tui assumes local service unless configured otherwise.
    ;; We'll follow client.rkt pattern if we had the URL.
    ;; Assuming localhost:8000 for now as default if not passed.
    (define connect-cmd
      (cmd (λ (ch)
             (with-handlers ([exn:fail? (λ (e) (async-channel-put ch (service-error-msg (exn-message e))))])
               ;; Attempt default connection
               (define client (connect-service! "http://localhost:8000"))
               (if client
                   (async-channel-put ch (service-connected-msg client))
                   (async-channel-put ch (service-error-msg "Failed to connect to localhost:8000")))))))
...
;; Start a periodic ticker command
  ...
  (define (login-cmd email password)
    (cmd (λ (ch)
           (with-handlers ([exn:fail? (λ (e) (async-channel-put ch (auth-error-msg (exn-message e))))])
  ...
  (define (create-session-cmd)
    (cmd (λ (ch)
           (with-handlers ([exn:fail? (lambda (e) (void))])
             ...
             ;; We don't have a specific msg for this in main-tui yet,
             ;; but we can update the model directly if we did this synchronously.
             ;; For async, we'd need a message. Let's do it sync in the success handler or ignore for now
             ;; main-tui generates session if missing.
             (void)))))
    ...
    (match evt
      [(tick-msg)
       ...
       (cond
         ...
         ;; Auto-transition to ready state after showing all frames
         ;; Intro complete - transition to auth or ready
         [else
          (if (and (model-service-client m) (model-api-key m))
              ;; Already authenticated, go to ready
              (let ()
      ...
      [(key-event _ _ _ _)
       (cond
         [(and (model-service-client m) (model-api-key m))
          (define new-input (struct-copy text-input-model (model-input m) [focused? #t]))
          (define sys-msg (cons 'system "Welcome! Type /help for commands, or start chatting."))
          (define sys-lines (render-msg-to-lines sys-msg (model-width m)))
          (values (struct-copy model m
                  ...
                  none)]
         [else
          (values (struct-copy model m [state STATE-AUTH]) none)])]
      ...
      [(service-connected-msg client)
       (values (struct-copy model m [service-client client]) none)]
  ...
  ;; Auth screen view
  (define (view-auth m width height)
    (define output-lines '())
  ...
  (module+ test
    ...
    (test-case "model struct creation"
               (define m (model STATE-INTRO        ; state
                                ...
                                'email              ; auth-field
                                (text-input-init)   ; email-input
                                (text-input-init)   ; password-input
                                (text-input-init)   ; display-name-input
                                #f                  ; error-message
                                #f))                ; service-client
