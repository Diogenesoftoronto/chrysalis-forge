...
async function loadBundledAssets() {
  // Generated by `bun run build:bundle-assets` and gitignored, so it may be
  // absent during typecheck/CI. A non-literal specifier keeps tsc from trying
  // to statically resolve an optional module that only exists after a build.
  const generatedModule = "./bundled-assets.generated.js";
  try {
    return await import(generatedModule);
  } catch {
    throw new Error(
